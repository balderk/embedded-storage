    ...
    S: MultiwriteNorFlash,
{
    fn write(&mut self, offset: u32, bytes: &[u8]) -> Result<(), Self::Error> {
            ...
            .overlaps(bytes, offset)
        {
            ...
            self.storage.write(
                page.start + aligned_start as u32,
                &self.merge_buffer[aligned_start..aligned_end],
            )?;

            self.storage.read(
                page.start + aligned_start as u32,
                self.merge_buffer[aligned_start..aligned_end].as_mut(),
            )?;

            let mut erase_all = false;

            for (i, d) in (offset..(offset + data.len())).zip(data.iter()) {
                if self.merge_buffer[i] != *d {
                    // error!("error at index {}. {:x} != {:x}", i, self.merge_buffer[i], *d);
                    erase_all = true;
                }
            }
            if erase_all {
                ...
                self.merge_buffer[offset..offset + data.len()].copy_from_slice(data);
                self.storage.write(page.start, self.merge_buffer)?;
            }

            /*
            if !data
                .iter()
                .zip(self.merge_buffer[offset..(offset + data.len())].iter())
                .all(|(v1, v2)| v1 == v2)
            {
                warn!("erasing page: {:x}", page.start);
                self.storage.erase(page.start, page.end())?;
